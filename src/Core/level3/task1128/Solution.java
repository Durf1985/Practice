...
public class Solution {
    ...
    public static void doMove(AbstractRobot robotFirst, AbstractRobot robotSecond) {
        //переменная асбтрактного класса принимает переменную наследника этого класса
        BodyPart attacked = robotFirst.attack();
        BodyPart defenced = robotFirst.defense();
        System.out.println(String.format("%s атаковал робота %s, атакована %s, защищена %s",
                robotFirst.getName(), robotSecond.getName(), attacked, defenced));
        //здесь не явно вызывается метод toString, потому что attacked это переменная класса BodyPart, а не абстрактного
        // робота в методе абстрактного .
    }
}
